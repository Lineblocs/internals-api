...
func (us *UserStore) GetSettings() (*model.APICredentials, error) {
	...
	var key string
	var value string
	apiCreds := model.APICredentials{ Credentials: make(map[string]string) }
	for results.Next() {
...
func (us *UserStore) IsAccountSuspended(workspaceId string) (bool, error) {
	var count int
	err := us.db.QueryRow(`SELECT COUNT(*) FROM workspaces_suspensions WHERE workspace_id = ? AND status != 'LIFTED' AND status != 'INITIATED' AND suspended_at IS NOT NULL`, workspaceId).Scan(&count)
